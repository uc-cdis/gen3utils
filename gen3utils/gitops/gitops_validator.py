...
from gen3utils.etl.etl_validator import get_all_nodes, validate_mapping
...
def val_gitops(data_dictionary, etl_mapping, gitops):
    ...
    error = validate_syntax(gitops_config)
    if error:
        raise AssertionError("Gitops failed to validate do to syntax error: {}".format(error))

    ok = True
    ok = validate_against_dictionary(gitops_config, data_dictionary)
    if not ok:
        raise AssertionError(
            "gitops validation failed. See errors in previous logs."
        )
...
def validate_syntax(gitops):
    ...
    for exp_config in configs:
        ...
        if manifest_mapping and val_mapping:
            checks = ["resourceIndexType", "resourceIdField", "referenceIdFieldInResourceIndex", "referenceIdFieldInDataIndex"]
            error = check_required_fields("explorerConfig.guppyConfig.manifestMapping", checks, manifest_mapping)
            if error:
                return error

    study_viewer = gitops.get("studyViewerConfig")
    if study_viewer:
        checks = ["dataType", "title", "titleField", "listItemConfig", "rowAccessor"]
        error = check_required_fields("studyViewerConfig", checks, study_viewer)
        if error:
            return error
    return None
...
def _validate_itemConfig(item_config, props, errors):
    ...
    errors = check_field_value("studyViewerConfig.blockFields.(list/single)ItemConfig", item_config.get("blockFields", []), props, errors)
    # for field in item_config.get("blockFields", []):
    #     if field not in props:
    #         errors.append(FieldError("Invalid field {} in studyViewerConfig.blockFields.(list/single)ItemConfig".format(field)))
    errors = check_field_value("studyViewerConfig.tableFields.(list/single)ItemConfig", item_config.get("tableFields", []), props, errors)
    # for field in item_config.get("tableFields", []):
    #     if field not in props:
    #         errors.append(FieldError("Invalid field {} in studyViewerConfig.tableFields.(list/single)ItemConfig".format(field)))
...
def _validate_studyViewerConfig_helper(viewer, type_prop_map, errors):
    ...
    if not props:
        errors.append(FieldError("Invalid field {} in studyViewerConfig.dataType".format(datatype)))
    ...
    for dtype, props in type_prop_map.items():
        if viewer['rowAccessor'] not in props:
            errors.append(FieldError("rowAccessor {} not found in index with type {}".format(viewer['rowAccessor'], dtype)))
...
def _validate_explorerConfig_helper(explorer_config, type_prop_map, errors):
    ...
    props = type_prop_map.get(datatype, [])
    if props:
        errors.append(FieldError("Invalid field {} in explorerConfig.guppyConfig.dataType".format(datatype)))

    tabs = explorer_config['filters']['tabs']
    errors = check_field_value("explorerConfig.filters.tabs.fields", tabs.get("fields", []), props, errors)
    # for field in tabs.get("fields", []):
    #     if field not in props:
    #         errors.append(FieldError("Invalid field {} in explorerConfig.filters.tabs.fields".format(field)))
    
    table = explorer_config['table']
    if table['enabled']:
        errors = check_field_value("explorerConfig.table.fields", table.get("fields", []), props, errors)
        # for field in table.get("fields", []):
        #     if field not in props:
        #         errors.append(FieldError("Invalid field {} in explorerConfig.table.fields".format(field)))
    ...
    if manifest_map and manifest_map.get("resourceIndexType"):
        ...
        if not resource_props:
            errors.append(FieldError("Invalid field {} in manifestMapping.resourceIndexType".format(manifest_map.get("resourceIndexType"))))
        elif manifest_map.get("resourceIdField") not in resource_props:
            errors.append(FieldError("Invalid field {} in manifestMapping.resourceIdField".format(manifest_map.get("resourceIdField"))))
        # TODO
...
def validate_explorerConfig(gitops, type_prop_map, errors):
    ...
    if exploreConfig and type(exploreConfig) == list:
        ...
        if file_exp_config:
            errors = _validate_explorerConfig_helper(file_exp_config, type_prop_map, errors)
    return errors
...
def validate_against_etl(gitops, mapping_file):
    ...
    if studyviewer:
        errors = validate_studyViewerConfig(studyviewer, mapping, errors)
    return errors
        ...
        if parent_props:
            for prop in parent_props:
                ...
                if str_props is not None:
                        ...
                        for p in props:
                            index_props.extend(p.split(":")[0] if p.find(":") != -1 else p)

        all_prop_map[index.get("doc_type")] = index_props
    return all_prop_map
...
def get_schema(model):
    return model.dictionary.schema
...
def validate_against_dictionary(gitops, data_dictionary):
    ...
    for item in graphql.get("boardCounts"):
        ...
        ok = assert_and_log(schema.get(node) is not None, "Node: {} in graphql.boardCounts not found in dictionary".format(node))
    ...
    for item in graphql.get("chartCounts"):
        ...
        idx = node_count.rfind("_")
        node = node_count[1:idx]        
        ok = assert_and_log(schema.get(node), "Node: {} in graphql.chartCounts not found in dictionary".format(node))

    for item in graphql.get("homepageChartNodes", []):
        node = item.get("node")        
        ok = assert_and_log(schema.get(node), "Node: {} in graphql.homepageChartNodes not found in dictionary".format(node))
