...
def test_blocks_validation(validation_config):
    """
    ...
    ok = blocks_validation(block_requirement, validation_config["block"])
    assert ok, "arborist 2.2.0 with deployment_version in arborist blcok should pass validation"
    ...
    ok = blocks_validation(block_requirement, validation_config["block"])
    assert not ok, "arborist 2.2.0 without deployment_version in arborist blcok should not pass validation"
    ...
    ok = blocks_validation(block_requirement, validation_config["block"])
    assert not ok, "hatchery without sidecar in hatchery blcok should not pass validation"
